    ...
    uniqueReferenceableIdentifiers(model, accept) {
        ...
        model.conditions.forEach(condition => {
            if (reported.has(condition.name)) {
                ...
                accept('error', `Condition has non-unique name '${condition.name}'. All names of Propositions and Conditions must be uniqued, to be properly referenced.`, { node: condition, property: 'name' });
                // show error on original referenceable
                let original = reported.get(condition.name);
                accept('error', `Object has non-unique name '${original.name}'. All names of Propositions and Conditions must be uniqued, to be properly referenced.`, { node: original, property: 'name' });
            }
        ...
        model.propositions.forEach(proposition => {
            if (reported.has(proposition.name)) {
                ...
                accept('error', `Proposition has non-unique name '${proposition.name}'. All names of Propositions and Conditions must be uniqued, to be properly referenced.`, { node: proposition, property: 'name' });
                // show error on original referenceable
